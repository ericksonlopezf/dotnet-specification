...
public sealed class MsSqlDialect : ISqlDialect
{
    ...
    /// <inheritdoc/>
    public string DialectName => "SQL Server";
    ...
    public SqlQuery Render(QueryModel model)
    {
        ...
        // OFFSET ... FETCH NEXT ... (SQL Server 2012+ when both Skip and Take)
        ...
        Dictionary<string, IReadOnlyList<string>> inExpansions)
    {
        switch (node)
        {
            ...
            case FullTextPredicateNode ft:
                sql.Append("CONTAINS(").Append(QuoteIdentifier(ft.ColumnName)).Append(", @").Append(ft.ParameterName).Append(')');
                break;
    ...
    private static string RenderOperator(SqlBinaryOperator op, string paramName) => op switch
    {
        ...
        _ => throw new NotSupportedException($"Operator '{op}' is not supported by SQL Server dialect.")
    };
}
