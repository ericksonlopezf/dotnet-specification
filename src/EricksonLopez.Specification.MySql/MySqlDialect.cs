...
public sealed class MySqlDialect : ISqlDialect
{
        ...
        Dictionary<string, IReadOnlyList<string>> inExpansions)
    {
        switch (node)
        {
            ...
            case FullTextPredicateNode ft:
                sql.Append("MATCH(").Append(QuoteIdentifier(ft.ColumnName)).Append(") AGAINST(@").Append(ft.ParameterName).Append(" IN NATURAL LANGUAGE MODE)");
                break;
    ...
    private static string RenderOperator(SqlBinaryOperator op, string paramName) => op switch
    {
        ...
        _ => throw new NotSupportedException($"Operator '{op}' is not supported by MySQL dialect.")
    };
}
