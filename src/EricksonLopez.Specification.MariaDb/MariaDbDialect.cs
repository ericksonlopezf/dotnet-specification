...
public sealed class MariaDbDialect : ISqlDialect
{
    ...
    /// <inheritdoc/>
    public string DialectName => "MariaDB";
        ...
        Dictionary<string, IReadOnlyList<string>> inExpansions)
    {
        switch (node)
        {
            ...
            case InPredicateNode inNode:
                ...
                {
                    // Empty collection: IN () is invalid SQL in MariaDB. Use always-false/always-true.
            ...
            case FullTextPredicateNode ft:
                sql.Append("MATCH(").Append(QuoteIdentifier(ft.ColumnName))
                   .Append(") AGAINST(@").Append(ft.ParameterName).Append(" IN NATURAL LANGUAGE MODE)");
                break;
    ...
    private static string RenderOperator(SqlBinaryOperator op, string paramName) => op switch
    {
        ...
        _ => throw new NotSupportedException($"Operator '{op}' is not supported by MariaDB dialect.")
    };
}
