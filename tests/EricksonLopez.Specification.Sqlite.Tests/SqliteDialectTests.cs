...
namespace EricksonLopez.Specification.Sqlite.Tests;

/// <summary>
/// Domain model used in SQLite dialect tests.
/// </summary>
...
/// <summary>
/// Unit tests for <see cref="SqliteDialect"/>.
/// </summary>
public sealed class SqliteDialectTests
{
    private readonly SqliteDialect _dialect = SqliteDialect.Default;
    ...
    [Fact]
    public void DialectName_IsSqlite()
    {
        _dialect.DialectName.Should().Be("SQLite");
    }
    ...
    public void Render_WithInPredicate_ProducesExpandedInClause()
    {
        ...
        result.Sql.Should().Contain("\"Category\" IN (@p1_0, @p1_1, @p1_2)");
        result.Sql.Should().NotContain("NOT IN");
        result.Parameters.Should().ContainKey("p1_0").WhoseValue.Should().Be("Electronics");
        result.Parameters.Should().ContainKey("p1_1").WhoseValue.Should().Be("Books");
        result.Parameters.Should().ContainKey("p1_2").WhoseValue.Should().Be("Clothing");
        result.Parameters.Should().NotContainKey("p1");
    }

    [Fact]
    public void Render_InPredicateNode_WithEmptyCollectionParameter_ProducesAlwaysFalse()
    {
        var model = new QueryModel
        {
            TableName = "T",
    ...
    [Fact]
    public void Render_InPredicateNode_NegatedWithEmptyCollectionParameter_ProducesAlwaysTrue()
    {
        var model = new QueryModel
        {
            TableName = "T",
    ...
    public void Render_EmptyCollectionNotInPredicate_ProducesAlwaysTrue()
    {
        ...
        result.Sql.Should().Contain("NOT (1 = 0)");
    }
    ...
    {
        SqliteDialect.Default.Should().NotBeNull();
    }
    ...
    public void Render_FullTextPredicate_RendersCorrectly()
    {
        ...
        var query = _dialect.Render(model);
        query.Sql.Should().Be("SELECT * FROM \"articles\" WHERE \"body\" LIKE '%' || @p1 || '%'");
    }
    ...
    public void Render_RemainingOperators_RendersCorrectSql(SqlBinaryOperator op, string expectedSuffix)
    ...
    public void Render_UnsupportedOperator_ThrowsNotSupportedException()
    {
        ...
        var act = () => _dialect.Render(model);
        act.Should().Throw<NotSupportedException>()
            .WithMessage("*Operator '999' is not supported by SQLite dialect.*");
    }
