...
namespace EricksonLopez.Specification.MariaDb.Tests;

public sealed class MariaDbDialectTests
{
    private readonly MariaDbDialect _dialect = MariaDbDialect.Default;

    [Fact]
    public void DialectProperties_ReturnExpectedValues()
    {
        _dialect.DialectName.Should().Be("MariaDB");
    ...
    public void QuoteIdentifier_QuotesWithBackticks(string input, string expected)
    {
    ...
    [Fact]
    public void Render_Orders_ProducesOrderByClause()
    {
        var model = new QueryModel
        {
            TableName = "customers",
    ...
    [Fact]
    public void Render_Pagination_WithTakeAndSkip()
    {
        var model = new QueryModel
        {
            TableName = "customers",
        ...
        query.Sql.Should().Be("SELECT * FROM `customers` LIMIT @_take OFFSET @_skip");
        query.Parameters["_take"].Should().Be(10);
        query.Parameters["_skip"].Should().Be(20);
    }

    [Fact]
    public void Render_Pagination_WithSkipOnly_EmitsMaxLimit()
    {
        var model = new QueryModel
        {
            TableName = "customers",
        ...
        query.Sql.Should().Be("SELECT * FROM `customers` LIMIT 18446744073709551615 OFFSET @_skip");
        query.Parameters["_skip"].Should().Be(50);
    }

    [Fact]
    public void Render_Pagination_WithTakeOnly_EmitsLimitWithoutOffset()
    {
        var model = new QueryModel
        {
            TableName = "customers",
        ...
        query.Sql.Should().Be("SELECT * FROM `customers` LIMIT @_take");
    ...
    public void Render_FullTextPredicate_RendersCorrectly()
    {
        ...
        var query = _dialect.Render(model);
        query.Sql.Should().Be("SELECT * FROM `articles` WHERE MATCH(`body`) AGAINST(@p1 IN NATURAL LANGUAGE MODE)");
    }
    ...
    public void Render_UnsupportedOperator_ThrowsNotSupportedException()
    {
        ...
        var act = () => _dialect.Render(model);
        act.Should().Throw<NotSupportedException>()
            .WithMessage("*Operator '999' is not supported by MariaDB dialect.*");
    }
