...
namespace EricksonLopez.Specification.MsSql.Tests;

public sealed class MsSqlDialectTests
{
    ...
    public void QuoteIdentifier_QuotesWithSquareBrackets(string input, string expected)
    {
    ...
    [Fact]
    public void Render_SelectDistinct_WithProjections_AndAlias()
    {
        var model = new QueryModel
        {
            TableName = "customers",
        ...
        query.Sql.Should().Be("SELECT DISTINCT [id], [name] FROM [customers] AS [c]");
    }

    [Fact]
    public void Render_SelectDistinct_WithTakeOnly_AndProjections()
    {
        var model = new QueryModel
        {
            TableName = "customers",
        ...
        query.Sql.Should().Be("SELECT DISTINCT TOP (@_take) [id] FROM [customers]");
        query.Parameters["_take"].Should().Be(5);
    }
    ...
    [Fact]
    public void Render_BinaryPredicates_RendersOperatorsCorrectly()
    {
        var model = new QueryModel
        {
            TableName = "customers",
    ...
    public void Render_Pagination_WithTakeAndSkip_WithOrders()
    {
        var model = new QueryModel
        {
            TableName = "customers",
            Filters = [],
            Orders = [new SqlOrderNode("name", OrderDirection.Descending)],
            Skip = 20,
            Take = 10,
            Parameters = []
        };
        ...
        query.Sql.Should().Be("SELECT * FROM [customers] ORDER BY [name] DESC OFFSET @_skip ROWS FETCH NEXT @_take ROWS ONLY");
        query.Parameters["_take"].Should().Be(10);
        query.Parameters["_skip"].Should().Be(20);
    }

    [Fact]
    public void Render_Pagination_WithTakeAndSkip_WithoutOrders_EmitsSelectNullOrder()
    {
        var model = new QueryModel
        {
            TableName = "customers",
        ...
        query.Sql.Should().Be("SELECT * FROM [customers] ORDER BY (SELECT NULL) OFFSET @_skip ROWS FETCH NEXT @_take ROWS ONLY");
        query.Parameters["_take"].Should().Be(10);
        query.Parameters["_skip"].Should().Be(20);
    }
    ...
    [Fact]
    public void Render_Pagination_WithSkipOnly_WithoutOrders()
    {
        var model = new QueryModel
        {
            TableName = "customers",
        ...
        query.Sql.Should().Be("SELECT * FROM [customers] ORDER BY (SELECT NULL) OFFSET @_skip ROWS");
        query.Parameters["_skip"].Should().Be(50);
    }

    [Fact]
    public void Render_Pagination_WithTakeOnly_EmitsTopWithoutOffset()
    {
        var model = new QueryModel
        {
            TableName = "customers",
        ...
        query.Sql.Should().Be("SELECT TOP (@_take) * FROM [customers]");
    ...
    public void Render_FullTextPredicate_RendersCorrectly()
    {
        var model = new QueryModel
        {
            ...
            Orders = [],
            Parameters = [new SqlParameter("p1", "database")]
        };

        var query = _dialect.Render(model);
        query.Sql.Should().Be("SELECT * FROM [articles] WHERE CONTAINS([body], @p1)");
    }
    ...
    public void Render_UnsupportedOperator_ThrowsNotSupportedException()
    {
        ...
        var act = () => _dialect.Render(model);
        act.Should().Throw<NotSupportedException>()
            .WithMessage("*Operator '999' is not supported by SQL Server dialect.*");
    }
