...
namespace EricksonLopez.Specification.MySql.Tests;

public sealed class MySqlDialectTests
{
    private readonly MySqlDialect _dialect = MySqlDialect.Default;

    [Fact]
    public void DialectProperties_ReturnExpectedValues()
    {
        _dialect.DialectName.Should().Be("MySQL");
    ...
    public void Render_UnsupportedOperator_ThrowsNotSupportedException()
    {
        ...
        var act = () => _dialect.Render(model);
        act.Should().Throw<NotSupportedException>()
            .WithMessage("*Operator '999' is not supported by MySQL dialect.*");
    }
