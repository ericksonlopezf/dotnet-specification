...
public sealed class PotentialClientSideEvaluationAnalyzerTests
{
    ...
    [Fact]
    public async Task Analyzer_UserDefinedOperatorExplicitInvocation_NoDiagnostic()
    {
        var code = """
            using System.Linq.Expressions;
            ...
            public sealed class SpecWithExplicitOperator : Specification<CustomMoney>
            {
                public override Expression<Func<CustomMoney, bool>> BuildExpression()
                {
                    return x => CustomMoney.op_Equality(x, default(CustomMoney));
                }
            }
            """;
