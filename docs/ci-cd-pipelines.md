...
This document defines the complete Continuous Integration and Continuous Deployment (CI/CD) architecture, quality gates, and automated validation workflows for `EricksonLopez.Specification`.
...
> [!NOTE]
> **Mutation Testing is NOT part of the main CI.** It runs as a separate scheduled job (`mutation-testing.yml`) every Monday at 04:00 UTC, or on manual dispatch to ensure fast CI cycle times.
...
Validates that `EricksonLopez.Specification` packages genuinely compile and run under Native AOT:

1. Publishes `tests/EricksonLopez.Specification.AotSmokeTest/` with `--PublishAot=true` and `-p:TreatWarningsAsErrors=true`.
2. Any IL2026 (RequiresUnreferencedCode) or IL3050 (RequiresDynamicCode) from the trimmer is treated as a build-breaking error.
...
Run locally:
```bash
pwsh scripts/run-stryker.ps1 -Config stryker-config.json
```
