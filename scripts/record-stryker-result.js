...
function evaluateScoreStatus(score, thresholds, foundReport = true) {
  const passedGate = score >= thresholds.break && foundReport;
  let statusLabel = '❌ FAILED';
  if (foundReport && score >= thresholds.high) {
    statusLabel = '✅ HIGH';
  } else if (foundReport && score >= thresholds.low) {
    statusLabel = '🟡 LOW';
  } else if (foundReport && score >= thresholds.break) {
    statusLabel = '🟠 WARNING';
  }
  return { statusLabel, passedGate };
}

function recordStrykerResult(targetDir, pkgName, configFile) {
  ...
  const { statusLabel, passedGate } = evaluateScoreStatus(score, thresholds, foundReport);
  ...
  // Write Step Summary with all mandatory fields
...
function main() {
  ...
  const pkgName = process.argv[3] || 'Specification';
  const configFile = process.argv[4] || 'stryker-config.json';
  recordStrykerResult(targetDir, pkgName, configFile);
}
