...
7. Consistent ImplicitUsings (disable) across Directory.Build.props and all .csproj.
...
$allMdFiles = Get-ChildItem -Path $RootDirectory -Recurse -Filter "*.md" | Where-Object {
    $_.FullName -notmatch "[\\/](bin|obj|\.git|node_modules|BenchmarkDotNet\.Artifacts|StrykerOutput|MEGA-AUDITORIA|results)[\\/]"
}
$badDocNames = 0
...
$badImplicit = 0
$propsFile = Join-Path $RootDirectory "Directory.Build.props"
if (Test-Path $propsFile) {
    $propsContent = [System.IO.File]::ReadAllText($propsFile)
    if ($propsContent -match "<ImplicitUsings>enable</ImplicitUsings>" -or $propsContent -notmatch "<ImplicitUsings>disable</ImplicitUsings>") {
        Write-Host "  ❌ ImplicitUsings must be disabled in Directory.Build.props" -ForegroundColor Red
        $violations++
        $badImplicit++
    }
}
foreach ($proj in $csprojs) {
    ...
    if ($content -match "<ImplicitUsings>enable</ImplicitUsings>") {
        Write-Host "  ❌ ImplicitUsings is enabled in $($proj.FullName)" -ForegroundColor Red
        $violations++
        $badImplicit++
    }
}
if ($badImplicit -eq 0) { Write-Host "  ✅ ImplicitUsings is consistently disabled across Directory.Build.props and all projects." -ForegroundColor Green }
...
if (Test-Path $publishWfPath) {
    ...
    foreach ($sp in $srcProjects) {
        if ($pubContent -notmatch [regex]::Escape($sp.Name)) {
            Write-Host "  ❌ Project '$($sp.Name)' is missing from .github/workflows/publish.yml pack steps." -ForegroundColor Red
            if (Get-Variable -Name "violations" -Scope 0 -ErrorAction SilentlyContinue) { $violations++ }
            if (Get-Variable -Name "Violations" -Scope 0 -ErrorAction SilentlyContinue) { if ($Violations -is [System.Collections.IList]) { $Violations.Add("Project '$($sp.Name)' missing from publish.yml") } else { $Violations++ } }
            if (Get-Variable -Name "FailedChecks" -Scope 0 -ErrorAction SilentlyContinue) { $FailedChecks++ }
            $gateErrors++
        }
    }
